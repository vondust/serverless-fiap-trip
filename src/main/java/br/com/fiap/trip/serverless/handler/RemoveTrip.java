...
public class RemoveTrip implements RequestHandler<TripEntity, HandlerResponse> {
	...
	public HandlerResponse handleRequest(final TripEntity trip, final Context context) {
		...
		if (tripRetrieved.isPresent()) {
			context.getLogger().log("[#] - Trip found " + tripRetrieved.get() + " deleting...");
			datastore.delete(tripRetrieved.get());
